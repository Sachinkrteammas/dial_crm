...
##### lead close status export ###


@login_required
def lead_close_status_export(request):
    menu_items = MenuItem.objects.filter(is_active=True).order_by('order')
    menu_tree = {}
    for item in menu_items:
        parent_id = item.parent_id
    ...
    if from_date_str and to_date_str:
        ...
        leads = LeadTable.objects.filter(
            ...
            lead_closer_status_new__in=[
                'closed_with_dealership',
                'closed_with_order',
                'closed_with_solution'
            ]
        )
        ...
        return FileResponse(
            file_stream,
            as_attachment=True,
            filename='Lead Closed Status.xlsx',
            content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
...
def fr_leads_export(request):
    ...
    if from_date_str and to_date_str:
        ...
        numbers = list(set([
            lead.calling_number for lead in leads_list if lead.calling_number
        ]))
        ...
        for idx, lead in enumerate(leads_list, start=1):
            ...
            if first_call and lead.created_at:
                ...
                if timezone.is_naive(first_call):
                    first_call = timezone.make_aware(
                        first_call, timezone.get_current_timezone()
                    )

                frt_seconds = (first_call - lead.created_at).total_seconds()
                frt_minutes = round(frt_seconds / 60, 2)

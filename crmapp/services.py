...
from .views import _fetch_apr_csv, _parse_apr_csv, _fetch_cdr_html, _parse_cdr_html  # reuse what already works
...
def save_cdr_report(start_date: str, end_date: str) -> int:
    start_date_obj = parse_date(start_date)
    end_date_obj = parse_date(end_date)
    if not start_date_obj or not end_date_obj:
        raise ValueError(f"Invalid date(s): start_date={start_date!r}, end_date={end_date!r}")
